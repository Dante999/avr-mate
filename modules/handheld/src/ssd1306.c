 ...
 * @return  none
******************************************************************************/
static void ssd1306_writedata(const uint8_t *data, uint8_t size)
{
	i2c_start_wait(SSD1306_ADDR + I2C_WRITE);
	i2c_write(0x40);
...
void ssd1306_init()
{
	ssd1306_writecmd(SSD1306_DISPLAYOFF); // 0xAE

	ssd1306_writecmd(SSD1306_SETDISPLAYCLOCKDIV); // 0xD5
	ssd1306_writecmd(0x80); // the suggested ratio 0x80

	ssd1306_writecmd(SSD1306_SETMULTIPLEX); // 0xA8

	ssd1306_writecmd(64 - 1);

	ssd1306_writecmd(SSD1306_SETDISPLAYOFFSET); // 0xD3
	ssd1306_writecmd(0x0);                      // no offset

	ssd1306_writecmd(SSD1306_SETSTARTLINE); // | 0x0);    // line #0

	ssd1306_writecmd(SSD1306_CHARGEPUMP); // 0x8D
	ssd1306_writecmd(0x14);               // using internal VCC

	ssd1306_writecmd(SSD1306_MEMORYMODE); // 0x20
	ssd1306_writecmd(0x00);               // 0x00 horizontal addressing

	ssd1306_writecmd(SSD1306_SEGREMAP | 0x1); // rotate screen 180

	ssd1306_writecmd(SSD1306_COMSCANDEC); // rotate screen 180

	ssd1306_writecmd(SSD1306_SETCOMPINS); // 0xDA
	ssd1306_writecmd(0x12);

	ssd1306_writecmd(SSD1306_SETCONTRAST); // 0x81
	ssd1306_writecmd(0xCF);

	ssd1306_writecmd(SSD1306_SETPRECHARGE); // 0xd9
	ssd1306_writecmd(0xF1);

	ssd1306_writecmd(SSD1306_SETVCOMDETECT); // 0xDB
	ssd1306_writecmd(0x40);

	ssd1306_writecmd(SSD1306_DISPLAYALLON_RESUME); // 0xA4

	ssd1306_writecmd(SSD1306_NORMALDISPLAY); // 0xA6

	ssd1306_writecmd(SSD1306_DEACTIVATE_SCROLL);

	ssd1306_writecmd(SSD1306_DISPLAYON); // switch on OLED
...
void ssd1306_fill(uint8_t symbol)
{
	...
	ssd1306_writecmd(SSD1306_SETCOLUMNADDR);
	ssd1306_writecmd(0);
	ssd1306_writecmd(SSD1306_LCDWIDTH - 1);

	ssd1306_writecmd(SSD1306_SETPAGEADDR);
	ssd1306_writecmd(0);
	ssd1306_writecmd(7);

	for (uint16_t i = 0; i <= LCD_BUFFER_SIZE; i = i + BLOCK_SIZE) {
		ssd1306_writedata(buffer, BLOCK_SIZE);
	}
}
 ...
 * @return  none
 ******************************************************************************/
void ssd1306_draw_tile_colpage(uint8_t column, uint8_t page,
                               const uint8_t *tile)
{

	ssd1306_writecmd(SSD1306_SETCOLUMNADDR);
	ssd1306_writecmd(column);
	ssd1306_writecmd(column + 7);

	ssd1306_writecmd(SSD1306_SETPAGEADDR);
	ssd1306_writecmd(page);
	ssd1306_writecmd(page);
...
void ssd1306_draw_buffer(const uint8_t *buffer)
{
	...
	ssd1306_writecmd(SSD1306_SETPAGEADDR);
	ssd1306_writecmd(0);
	ssd1306_writecmd((SSD1306_LCDHEIGHT / 8) - 1);

	for (uint16_t i = 0; i < LCD_BUFFER_SIZE; i = i + BLOCK_SIZE) {
		ssd1306_writedata(buffer + i, BLOCK_SIZE);
	}
}
